...
* **Namespace Isolation:** Prefix all Redis keys with their owning module and entity to prevent collisions (e.g., `auth:session:user_123`, `feed:post_456:likes`).

## Caching Strategies
* **When to Cache:** Cache only heavily read, rarely updated data (e.g., public community profiles, configuration lists). Do not cache highly transactional data that requires strict consistency unless using explicit distributed locking.
