---
name: ui-ux-pro-max
description: UI/UX design intelligence with searchable database. Contains 84 UI styles, 161 color palettes, 73 font pairings, 99 UX guidelines, and 25 chart types across 17 tech stacks. Searchable database with priority-based recommendations.
---
...
- **Stack**: Detect from project context or ask the user
...
**Example:**
```bash
python3 src/ui-ux-pro-max/scripts/search.py "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```
...
```bash
python3 src/ui-ux-pro-max/scripts/search.py "atlantafx primer enterprise theme" --stack javafx
python3 src/ui-ux-pro-max/scripts/search.py "enterprise tableview density permission" --stack javafx
```
...
```bash
python3 src/ui-ux-pro-max/scripts/search.py "AI search tool modern minimal" --design-system -p "AI Search"
```
...
python3 src/ui-ux-pro-max/scripts/search.py "search loading animation" --domain ux
```

### Step 4: Stack Guidelines

```bash
python3 src/ui-ux-pro-max/scripts/search.py "list performance navigation" --stack react-native
```
...
- Default icon library: **Phosphor (`@phosphor-icons/react`)**. The icons listed in `src/ui-ux-pro-max/data/icons.csv` are commonly recommended icons, not the full set.
- When the recommended list doesn't have a suitable icon:
  - **Prefer picking any semantically better icon from the full Phosphor set**;
  - If Phosphor also lacks an ideal option, use **Heroicons (`@heroicons/react`)** as a fallback, maintaining style consistency (linear/filled, stroke weight, corner style).
