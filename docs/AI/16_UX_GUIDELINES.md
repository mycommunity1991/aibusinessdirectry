...
- An unclaimed listing (`S-08`, `S-09`) must be **visually distinct at a glance** — not just a small badge a user could miss — since showing it as indistinguishable from a claimed, verified listing would mislead the customer. **Resolved pattern** (`13_OPEN_DECISIONS.md` item 4): a full-width banner across the top of both the provider card (`S-08`) and the provider profile (`S-09`), in the Warning color (`#F59E0B`, already reserved for "unclaimed-listing label" in this doc's color table), reading "Unclaimed — Is this your business? Claim it" with an inline CTA that opens the Claim flow (`S-21`, Flow 3). This is a design decision only — building it is deferred alongside `CLM-001` (item 3, Sprint 6), since no unclaimed listing exists in the product until that story ships.
