...
## Sprint 9 — Outcome & Reviews (In Progress — 1 of 2 stories done — Milestone ML9)

Scope covers the platform's only conversion signal, the Outcome Tag (`REV-001`), followed by the anchor-verified
review model built on top of it (`REV-002`, not yet started).

| Story | Description | Status |
|--------|-------------|--------|
| REV-001 | Tell the platform whether I hired a provider | ✅ Done |
| REV-002 | Leave a verified review after a successful hire | ⏳ Next (depends on REV-001, pending CTO's explicit "Start REV-002") |
...
`REV-002` ("leave a verified review after a successful hire") is the next story in this epic (ML9-EP01), depends
on `REV-001`, and is not yet started — it will finally build `reviews`/`provider_rating_summaries`, resolving
`13_OPEN_DECISIONS.md` item 14. See `docs/AI/SESSION_HANDOFF.md` for its full verbatim ACs and readiness status.
...
**This completes Sprint 8 and Milestone ML8 in full — 2 of 2 stories done: `MAT-001`, `CON-001`.** The next story
is Sprint 9's first story — **not identified in this document.** The orchestrator must look it up in
`docs/AI/Project_Tracker.xlsx` and record it here (and in `docs/AI/SESSION_HANDOFF.md`) before the next planning
session can proceed without a tracker read. No engineering agent should plan or start any Sprint 9 story without
