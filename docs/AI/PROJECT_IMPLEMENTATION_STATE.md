...
dashboard"), is the next startable story — see Section 17; gated on an explicit CTO "Start X" instruction.
...
Marketplace Operations — In Progress (Sprint 11, Milestone ML11, 1 of 2 stories done: `ADM-001` shipped the new
`administration.unmatched_query_reports` table and its admin review/action workflow — `administration`'s first
`api.py` — plus the conversation-transcript embedding on the existing manual-match list endpoint. `tester`
verified all 7 ACs pass with zero gaps and zero regressions; `architect` returned zero blocking findings, no
fix-and-recheck round needed. Backend-only. `ADM-002` is Sprint 11's second and final story, not yet started)

**Sprint 11 (Marketplace Operations) is now 1 of 2 stories done: `ADM-001`.**
