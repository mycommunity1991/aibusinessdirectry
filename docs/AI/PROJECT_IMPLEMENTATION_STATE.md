...
- Conversation / AI Intake — **no longer a gap, fully shipped for its Sprint 7 scope**: `AI-001` lets a customer
  ...
  queue this story's `GET /admin/search/manual-matches` is backend-API-only for), and a merit-based ranking
  algorithm (the Review domain hasn't shipped, so there's no real rating signal to rank by yet).
- Search & Matching — **now shipped through `AI-002`, not merely DIR-001's structured browse**: a structured,
  proximity-ordered (non-AI) directory browse exists (`GET /search/providers`/`GET /search/categories`, the
  mobile Search Filters/Search Results screens), searching only self-registered, `is_discoverable=true` providers
  via free-text category exact-match (`13_OPEN_DECISIONS.md` item 1's interim posture, ADR-027) and the
  now-confirmed-shipped `cube`/`earthdistance` GiST indexes (`04_DATABASE.md` Section 13). `AI-002` has since
  shipped the real, persisted `search.search_requests`/`provider_matches`/`search_event_log` tables (DIR-001's
  own query remains deliberately unchanged and still writes none of them, `Plan_S06_DIR-001.md` Decision 5) and
  the Wizard-of-Oz manual-match fallback for low-confidence AI-Conversation sessions. **Not yet built:**
  AI-ranked/merit-based results (a future story, once the Review domain ships real rating data) — nearest-first
  (DIR-001's existing order) is reused unchanged for now, honestly, not dressed up as more precise than it is.
- Contact View
...
**With Sprint 7 and Milestone ML7 now both fully complete, the next sprint/milestone has not been planned or
started by this closeout.** Candidates this closeout surfaces but does not plan: `ADM-001`/`ADM-002` (the admin
operations dashboard, Sprint 11 per the current Tracker structure — now with three real backend admin queues
across `administration` to design a UI against, most recently `AI-002`'s manual-match queue), and real LLM
vendor selection (`13_OPEN_DECISIONS.md` item 13, which nothing currently shipped depends on resolving). Which
sprint to plan next is a product/sequencing call for the CTO, not one this document makes on its own — **no
engineering agent should plan or start a new sprint's story without that explicit direction**, matching this
