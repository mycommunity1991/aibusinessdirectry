...
Walkthrough_S11_ADM-002.md` for the full account. **The next sprint/milestone's first story has not yet been
identified — a fresh lookup against `docs/AI/Project_Tracker.xlsx` is needed before planning begins; see Section
17.**
...
**The next sprint/milestone's first story has been identified via a fresh tracker lookup: `ENG-001`
("Receive marketplace notifications in my preferred channel"), Sprint 12 (Theme: "Engagement & Trust"),
Milestone `ML12`, Epic `ML12-EP01`.** Priority High; depends on `CON-001`/`VER-002`/`REV-001` (all already
shipped); status ⏳ Planned. Its 8 verbatim ACs require: `notifications`/`notification_preferences`/
`notification_delivery` tables via migration; a single `NotificationSender` interface with WhatsApp/SMS/Email
adapters (typed-error failures, never a silent no-op); trigger wiring for new Contact View → Provider,
verification status change → Provider, manual match assignment created → Admin, and outcome-tag prompt →
Customer; a hard-stop preference/mute check before every send; a badge/inbox-vs-immediate-push distinction for
non-urgent vs. time-sensitive events; a Notifications Inbox screen (New/Earlier grouping, deep-links); an
idempotency key preventing duplicate sends; and tests for preference-disabled blocking and idempotent-retry
non-duplication. Sprint 12's second story is `ENG-002` ("share a provider profile and verify an arrival,"
depends on `CON-001`) — out of scope for `ENG-001`. **A genuine discrepancy must be resolved during planning,
not assumed:** `ENG-001`'s description frames this as building the Notification domain "end to end," but
`NotificationService` already exists and is already called synchronously from `ContactService`/`OutcomeTagService`
(`REV-001`/`CON-001`, `ADR-050`) with no scheduling infrastructure and — as far as this codebase's history shows
— no dedicated `notifications`/`notification_preferences`/`notification_delivery` schema, no multi-channel
adapters, and no idempotency key. This mirrors the same class of tracker-text-vs-reality gap already found and
corrected twice this project (`unmatched_query_reports` before `ADM-001`; `feature_flags`/`system_settings`
before `ADM-002`) — `tech-lead` must verify `NotificationService`'s actual current state directly against the
code before finalizing `ENG-001`'s Plan scope, not trust either the tracker text or this summary. No engineering
agent should start planning or implementing `ENG-001` without an explicit CTO "Start ENG-001" instruction,
matching this document's own established convention.
