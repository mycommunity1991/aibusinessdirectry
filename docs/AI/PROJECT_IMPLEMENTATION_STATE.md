...
`docs/AI/Project_Tracker.xlsx` is needed before any planning begins — mirroring this document's own established
convention at every previous milestone transition. No engineering agent should plan or start any further story
without first performing that lookup, and then without an explicit CTO "Start X" instruction, matching this
document's own established convention.
